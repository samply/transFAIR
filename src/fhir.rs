...
impl ParameterExt for Vec<Option<ParametersParameter>> {
    fn get_param_by_name(&self, name: &str) -> Option<&ParametersParameter> {
        self.iter().flatten().find(|p| p.name.value.as_deref() == Some(name))
    }
}

pub mod fhir_json {
    use fhirbolt::serde::{DeserializeResourceOwned, SerializeResource, };
    use serde::{de::DeserializeSeed, Deserializer, Serialize, Serializer};

    
    pub fn serialize<T: SerializeResource, S: Serializer>(ser: S, input: &T) -> Result<S::Ok, S::Error> {
    ...
    pub fn deserialize<'de, T: DeserializeResourceOwned, D: Deserializer<'de>>(deser: D) -> Result<T, D::Error> {
